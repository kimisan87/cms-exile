...
class Main extends CMS_Controller
{
    ...
    public function static_page($navigation_name)
    {
	 ...
	 $pass = $this->input->post('password');
	  ...
	  if ($this->cms_do_login($identity, $pass)) {
    ...
    public function management()
    {
	   ...
	   //$this->template->admin('admin_main',$data,'main_management');
    }
    ...
    public function navigation()
    {
        ...
        $crud->columns('navigation_name', 'parent_id', 'title', 'active', 'only_content', 'is_static', 'authorization_id', 'groups','icon');
        $crud->edit_fields('navigation_name', 'parent_id', 'title', 'page_title', 'page_keyword', 'description', 'active', 'only_content', 'is_static', 'static_content', 'default_theme', 'url', 'authorization_id', 'groups','icon');
        $crud->add_fields('navigation_name', 'parent_id', 'title', 'page_title', 'page_keyword', 'description', 'active', 'only_content', 'is_static', 'static_content', 'default_theme', 'url', 'authorization_id', 'groups','icon');
        $crud->field_type('active', 'true_false');
...
//EDIT master editable menu
  public function widget_side_admin_nav($caption = 'Complete Menu', $first = TRUE, $navigations = NULL){
	...
	*/
	 foreach($navigations as $nav){
		if(($nav['allowed'] && $nav['active'])){
		if($nav['have_allowed_children']){
		$links .= '<li class="has-sub"><a href="javascript:;">
		...
		foreach($nav['child'] as $child){
		 $links .= '<li><a href="'.$child['url'].'">'.$child['title'].'</a></li>';
		}		
		$links .='</ul></li>';
			//do if got child
		}else{
		$links .= '<li class="start">
					<a href="'.$nav['url'].'">
					<i class="icon-'.$nav['icon'].'"></i> 
					<span class="title">'.$nav['title'].'</span>
	 ...
	 $main_nav = '<ul><li class="has-sub">'.$links.'</li></li>';

		$this->cms_show_html($main_nav);
		echo '<li style="color:white;padding-left:4%;"><h4>QuickLinks</h4></li>';
	...
	public function eqool_main_menu($caption = 'Complete Menu', $first = TRUE, $navigations = NULL){
	   if(!isset($navigations)){
            $navigations = $this->cms_navigations();
        }
        if(count($navigations) == 0) return '';
...
//TODO:edit HERE
    private function widget_quicklink(){
        ...
        $html = '<div id="_cms_widget_14"><ul>';
        foreach($quicklinks as $quicklink){
			$html .= '<li class="start">
					<a href="'.$quicklink['url'].'">
					 
					<span class="title">'.$quicklink['title'].'</span>
					<span class="selected"></span>
					</a>
				</li>';
